...
> ビン幅と位置ズレ範囲の指定は runcard の PositionWindow, PositionError, InternalMode で決定される。
...
InternalMode    = 2                 # InteranlMode=1/2 を推奨。0 は廃止予定。下記の「接続判定で許容する範囲」を参照。
Histo           = 1 10 -1000 +1000 -1000 +1000 # --histo で作成するヒストグラムの bin-width xmin xmax ymin ymax 指定 ( 先頭は 1 に固定 )。
                                               # 補正後の位置ズレ０が原点であり、スリップの検知・補正に使える。
```
#### 接続判定で許容する範囲
